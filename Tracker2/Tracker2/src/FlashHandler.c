...
extern uint32_t lsmRowsRecorded,varRowsRecorded,GPSRecorded;
...
void FlashInit()
{
//	asf_spi_init_pins();	done in init.c
...
void eraseAllFlash()
{
	...
	notBusy();
	NextPageAvailable = 0;
	SerialWrapSendByte(CleanFlashAck); // send this AFTER chip erase
}
...
void WritePageBufferToFlash()
{
	...
	SerialWrapSendByte(TelemBlockToFLash);	
	updatePageBitMap();
}
...
void sendFlashOffsetAndLastRecordsToHost()
{
    ...
    uint8_t printbuf[50];	
	int pflength;
	pflength = snprintf((char *)printbuf,50,"%lu;%lu;%lu;%lu;",NextPageAvailable,GPSRecorded,lsmRowsRecorded,varRowsRecorded);	
	SerialWrapSendByte(StartFlashOffset);
	cm = SerialWrap(printbuf,pflength,&cookedlength);
	udi_cdc_write_buf(cm,cookedlength);
	SerialWrapSendByte(EndFlashOffset);
}
...
void SendFlashDataToHost()
{
    ...
    while (addre < NextPageAvailable)		
	{
		...
		while (valid && offset < PAGE_SIZE)  // no more data
		{
			...
			if (valid)
			{
				SerialWrapSendByte(StartFlashDataToHost);
				cm = SerialWrap(&fromFlash[offset],length,&cookedlength);
				udi_cdc_write_buf(cm,cookedlength);
				SerialWrapSendByte(EndFlashDataToHost);	
			}
			offset += length;			
		}
		
		SerialWrapSendByte(FlashPageCountToHost);
	...
	SerialWrapSendByte(NoMoreFlashData);         
}
