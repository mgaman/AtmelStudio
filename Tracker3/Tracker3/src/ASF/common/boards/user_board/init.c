...
//
uint32_t gsmfifoerrors;
ISR(USARTD0_RXC_vect)
{
	if (fifo_push_uint8(&GSM_rx_fifo,usart_getchar(USART_GSM)) != FIFO_OK)
		gsmfifoerrors++;
}
...
//
uint32_t gpsfifoerrors;
extern bool GPSreadyToReceive;
ISR(USARTE0_RXC_vect)
{
	uint8_t c = usart_getchar(USART_GPS);
	if (GPSreadyToReceive)
	{
		if (fifo_push_uint8(&GPS_rx_fifo,c) == FIFO_ERROR_OVERFLOW)
			gpsfifoerrors++;
	}
}
