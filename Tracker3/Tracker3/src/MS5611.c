...
twi_package_t packet = {
	.addr_length = 0,
	.addr = {0,0,0},
	.chip = MS5611_SLAVE_ADDRESS, // issue to slave
	.no_wait = false,// Wait if bus is busy
	.buffer = 0,
	.length = 0
	};
//uint16_t C[8]; // calibration coefficients
struct sMS5611_Calibration MS5611CalibData;
...
bool MS561101BA03Reset()
{
	...
	if (master_status == STATUS_OK)
	{
		...
		memset(MS5611CalibData.C,0xff,sizeof(MS5611CalibData.C));
		for (i=0;i<sizeof(MS5611CalibData.C)/sizeof(uint16_t);i++)
//		i = 0;
//		while (master_status == STATUS_OK && i < sizeof(C)/sizeof(uint16_t))
		{
			packet.addr[0] = CMD_PROM_RD+ (i*2);
			packet.addr_length = 1;
			packet.buffer = &MS5611CalibData.C[i];
			packet.length = sizeof(uint16_t);
			master_status = twi_master_read(&TWI_MASTER, &packet);	
		//	i++;		
		}
		n_crc=crc4(MS5611CalibData.C);    // calculate the CRC
		rc = true;
	}
	return rc;
...
void MS561101BA03FormatAltimeter(uint32_t D1,uint32_t D2,double *Temp,double *Press)
{
	...
	dT=D2-MS5611CalibData.C[4]*pow(2.0,8.0);
	OFF=MS5611CalibData.C[1]*pow(2.0,17.0)+dT*MS5611CalibData.C[3]/pow(2.0,6.0);
	SENS=MS5611CalibData.C[0]*pow(2.0,16.0)+dT*MS5611CalibData.C[2]/pow(2.0,7.0);
	
	*Temp=(2000+(dT*MS5611CalibData.C[5])/pow(2.0,23.0))/100;   // temperature * 100
